...
export function ProdutoDetalhesModal({ produto, isOpen, onClose }: ProdutoDetalhesModalProps) {
  ...
  const carregarTabelasPrecos = async () => {
    ...
    try {
      ...
      // Buscar todas as tabelas (AS_TABELA_PRECOS) para ter a lista completa
      const tabelasSankhya = await OfflineDataService.getTabelasPrecos()
      
      // Buscar também as configuradas (AD_TABELASPRECOSCONFIG) para saber quais exibir prioritariamente
      const tabelasConfig = await OfflineDataService.getTabelasPrecosConfig()

      // Obter o parceiro selecionado do sessionStorage (como no resto do sistema de pedidos)
      ...
      // Criar a lista de tabelas para exibir
      let tabelasParaExibir: any[] = []

      // 1. Prioridade: Tabela vinculada ao parceiro (via CODTAB -> NUTAB)
      if (nutabParceiro) {
      ...
      // 2. Adicionar tabelas configuradas (AD_TABELASPRECOSCONFIG)
      ...
      const precosPromises = tabelasParaExibir.map(async (tabela: any) => {
        ...
        if (precos && precos.length > 0) {
          ...
          // Prioridade para VLRVENDA (maiusculo), depois vlrVenda (camelCase), depois PRECO
